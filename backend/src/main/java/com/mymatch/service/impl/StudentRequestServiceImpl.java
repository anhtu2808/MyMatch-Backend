...
public class StudentRequestServiceImpl implements StudentRequestService {
    ...
    public StudentRequestResponse createStudentRequest(StudentRequestCreationRequest req) {
        ...
        if (req.getSkillIds() != null && !req.getSkillIds().isEmpty()) {
            ...
            for (Skill s : skills) {
                studentRequest
                        .getSkills()
                        .add(StudentRequestSkill.builder()
    ...
    public StudentRequestResponse updateStudentRequest(Long id, StudentRequestUpdateRequest req) {
        ...
        studentRequest.setCourse(courseRepository
                .findById(req.getCourseId())
                .orElseThrow(() -> new AppException(ErrorCode.COURSE_NOT_FOUND)));
        studentRequest.setSemester(semesterRepository
                .findById(req.getSemesterId())
                .orElseThrow(() -> new AppException(ErrorCode.SEMESTER_NOT_FOUND)));
        studentRequest.setCampus(campusRepository
                .findById(req.getCampusId())
                .orElseThrow(() -> new AppException(ErrorCode.CAMPUS_NOT_FOUND)));
        if (req.getSkillIds() != null) {
            ...
            studentRequest
                    .getSkills()
                    .removeIf(srs -> !newIds.contains(srs.getSkill().getId()));
            // thêm những cái mới
    ...
    @Override
    public PageResponse<StudentRequestResponse> getAllStudentRequests(
